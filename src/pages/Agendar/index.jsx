...
const Agendar = () => {
  ...
  const diasMap = {
    ...
    if (user && user.id) {
      axios.get(`https://fisiopilattes-production.up.railway.app/reservas/${user.id}`)
        .then(response => {
  ...
  useEffect(() => {
    axios.get('https://fisiopilattes-production.up.railway.app/reservas')
      .then(response => {
  ...
  const atualizarStatus = (id, status) => {
    axios.patch(`https://fisiopilattes-production.up.railway.app/reservas/${id}`, { status })
      .then(() => {
...
useEffect(() => {
  ...
  }, 5000 ); // Envia o e-mail a cada 10 segundos (10000 ms)
...
const handleReserva = async (e) => {
  ...
  try {
    ...
    const response = await axios.post('https://fisiopilattes-production.up.railway.app/reservas', {
      nome: user.nome,
  ...
  const handleFaltar = async (id) => {
    ...
    try {
        await axios.put(`https://fisiopilattes-production.up.railway.app/reservas/solicitar/${id}`, { 
            status: 'ausente', 
            motivoFalta: tempo 
        });
...
const handleSalvarEdicao = async () => {
  ...
  try {
    ...
    const response = await axios.patch(`https://fisiopilattes-production.up.railway.app/reservas/editar/${reservaId}`, {
      dia: diaCorrigido,
      horario: novoHorario,
  ...
  const enviarReservas = async () => {
    ...
    try {
      const enviarReserva = await Promise.all(reservasTemporarias.map(async (reserva) => {
        await axios.post('https://fisiopilattes-production.up.railway.app/reservas', {
          nome: user.nome,
          sobrenome: user.sobrenome,
          email: user.email,
          telefone: user.telefone,
            ...
            {reservasOrdenadas.length > 0 ? reservasOrdenadas.map(reserva => (
              <ReservaItem key={reserva.id}>
                <MensagemStatus status={reserva.status}>
                  ...
                  <strong>Horário:</strong> {reserva.horario} - {reserva.horarioFinal}<br />
                  ...
                  {reserva.status === 'confirmado' ? (
                    <p>Confirmado: {reserva.dia} às {reserva.horario}</p>
                  ) : (
                    <>
                      <br />
