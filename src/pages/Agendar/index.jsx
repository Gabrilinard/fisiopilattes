...
const Agendar = () => {
  ...
  const diasMap = {
    ...
    if (user && user.id) {
      axios.get(`https://localhost:4000/reservas/${user.id}`)
        .then(response => {
  ...
  useEffect(() => {
    axios.get('https://localhost:4000/reservas')
      .then(response => {
  ...
  const atualizarStatus = (id, status) => {
    axios.patch(`https://localhost:4000/reservas/${id}`, { status })
      .then(() => {
...
const handleReserva = async (e) => {
  ...
  try {
    ...
    const response = await axios.post('https://localhost:4000/reservas', {
      nome: user.nome,
  ...
  const handleFaltar = async (id) => {
    ...
    try {
        await axios.put(`https://localhost:4000/reservas/solicitar/${id}`, { 
            status: 'ausente', 
            motivoFalta: tempo 
        });
...
const handleSalvarEdicao = async () => {
  ...
  try {
    ...
    const response = await axios.patch(`https://localhost:4000/reservas/editar/${reservaId}`, {
      dia: diaCorrigido,
      horario: novoHorario,
  ...
  const enviarReservas = async () => {
    ...
    try {
      const enviarReserva = await Promise.all(reservasTemporarias.map(async (reserva) => {
        await axios.post('https://localhost:4000/reservas', {
          nome: user.nome,
          sobrenome: user.sobrenome,
          email: user.email,
          telefone: user.telefone,
