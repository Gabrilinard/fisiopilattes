...
const AdminDashboard = () => {
  ...
  const buscarUsuariosLogados = async () => {
    try {
      const response = await axios.get('https://localhost:4000/usuarios/logados'); // Substitua com a rota do seu backend
  ...
  const buscarUsuarioPorId = async (id) => {
    try {
      const response = await axios.get(`https://localhost:4000/usuarios/solicitarDados/${id}`);
  ...
  const removerReserva = (id) => {
    if (window.confirm("Tem certeza que deseja remover esta reserva?")) {
      axios.delete(`https://localhost:4000/reservas/${id}`)
        .then(() => {
  ...
  const negarReserva = (reserva) => {
    ...
    axios.patch(`https://localhost:4000/reservas/negado/${reserva.id}`, { 
      status: 'negado', 
